  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-[#2B3674]">Peta Gudang & Lokasi</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {safeLocations.map((loc) => {
          ...
          return (
            <div
              key={loc.id}
              className="group rounded-[20px] bg-white p-6 shadow-[14px_17px_40px_4px_rgba(112,144,176,0.08)] transition-all hover:-translate-y-1 hover:shadow-[14px_20px_40px_8px_rgba(112,144,176,0.12)] border border-transparent hover:border-[#4318FF]/10 flex flex-col h-full"
            >
              <div className="flex items-start justify-between mb-6">
                <div className="flex items-center gap-3">
                  <div className="flex h-12 w-12 items-center justify-center rounded-full bg-[#F4F7FE] text-[#4318FF] transition-colors group-hover:bg-[#4318FF] group-hover:text-white">
                    <MapPin className="h-6 w-6" />
                  </div>
                  <div>
                    <h3 className="text-lg font-bold text-[#2B3674]">{loc.name}</h3>
                    <p className="text-xs font-medium text-[#A3AED0]">ID: {loc.id.split('-')[0]}</p>
                  </div>
                </div>
              </div>

              <div className="flex-1">
                <div className="mb-4 flex items-center justify-between rounded-xl bg-[#F4F7FE] p-4">
                  <div className="flex items-center gap-2">
                    <Box className="h-5 w-5 text-[#4318FF]" />
                    <span className="text-sm font-bold text-[#2B3674]">Total Kapasitas</span>
                  </div>
                  <span className="text-xl font-black text-[#4318FF]">{totalItems}</span>
                </div>

                <div className="grid grid-cols-2 gap-2 mb-6">
                  <div className="rounded-xl border border-slate-100 p-3">
                    <p className="text-xs font-medium text-[#A3AED0]">Kondisi Baik</p>
                    <p className="text-lg font-bold text-[#01B574]">{totalGood}</p>
                  </div>
                  <div className="rounded-xl border border-slate-100 p-3">
                    <p className="text-xs font-medium text-[#A3AED0]">Kondisi Rusak</p>
              ...
              <div className="mt-auto pt-4 border-t border-slate-100">
                <Link href={`/locations/${loc.id}`} className="w-full">
                  <Button className="w-full bg-[#F4F7FE] text-[#4318FF] hover:bg-[#4318FF] hover:text-white font-bold rounded-xl h-11 transition-all group-hover:shadow-md">
                    Lihat Isi Gudang
